...
export default function IndicatrixFigure() {
  ...
  const prm = useMemo<OnePoleParams>(() => ({ ...SEED, lambda, pole }), [lambda, pole])
  const member = useMemo(() => toMember(prm), [prm])

  const loop = useMemo(() => indicatrixLoop(member).map(tri), [member])
        ...
        <span className="flex items-center gap-3 flex-wrap justify-center">
          ...
          <label className="flex items-center gap-1">
            ...
            <input
              ...
              onChange={(e) => setPole(Number(e.target.value))}
              className="w-24"
      ...
      caption={
        <>
          ...
          <span className="text-slate-400">
            ...
            a pole at all. Nothing here is normalised — ‖T‖ − 1 stays at machine zero because the sphere
            is an identity, not a projection. Drag the background to rotate.
          </span>
        </>
      }
    >
      ...
      <Curve3D points={loop} color={FIG.color.curveMuted} width={1.5} />
