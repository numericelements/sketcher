...
import { IBSplineR1toR2 as IBSplineR1toR2 } from "./IBSplineR1toR2"
 ...
 */
export abstract class BaseBSplineR1toR2 implements IBSplineR1toR2 {

    protected readonly _controlPoints: readonly Vector2d[]
     ...
     */
    protected abstract factory(controlPoints: readonly Vector2d[], knots: readonly number[]): BaseBSplineR1toR2

    computeDegree() {
        let degree = this._knots.length - this._controlPoints.length - 1
        if (degree < 0) {
            throw new Error("Negative degree BSplineR1toR2 are not supported")
        }
        return degree
    }
    ...
    abstract getDifferentialProperties(): BSplineR1toR2DifferentialPropertiesInterface

    /**
     * Return a deep copy of this b-spline
     */
    
    abstract clone() : BaseBSplineR1toR2
     ...
     */
    abstract extract(fromU: number, toU: number) : BaseBSplineR1toR2

}

export function deepCopyControlPoints(controlPoints: readonly Vector2d[]): Vector2d[] {
    let result: Vector2d[] = []
